...
# ---------- BANKS & CARDS ----------
@app.route('/api/banks', methods=['GET', 'OPTIONS'])
def get_banks():
    ...
    try:
        ...
        print(f"Error in get_banks: {str(e)}")
        return jsonify({"error": str(e)}), 500


# ---------- OFFERS ----------
@app.route('/api/offers', methods=['GET', 'OPTIONS'])
def get_offers():
    if request.method == 'OPTIONS':
        return '', 200
...
# ---------- MERCHANTS ----------
@app.route('/api/merchants', methods=['GET', 'OPTIONS'])
def get_merchants():
    if request.method == 'OPTIONS':
        return '', 200
...
@app.route('/api/my-cards', methods=['GET', 'OPTIONS'])
def get_my_cards():
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        ...
        print(f"Error in get_my_cards: {str(e)}")
        return jsonify({"error": str(e)}), 500


@app.route('/api/my-cards', methods=['POST', 'OPTIONS'])
def add_my_card():
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        ...
        print(f"Error in add_my_card: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500
